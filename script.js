window.onload = function() {
  ...
  let lastDeleted = null, lastDeletedIndex = null;

  function getMonthStr(dt = new Date()) {
    return dt.toLocaleString('default', { month: 'long', year: 'numeric' });
  }
  function getDay(dt = new Date()) {
  ...
  function openModal(isEdit = false, idx = null) {
    ...
    saveBtn.disabled = true; noteInput.value = '';
    if (isEdit && idx !== null) {
  ...
  form.onsubmit = (e) => {
    ...
    if (!amount || !category || !note || note.length > 15) return false;
    let edIdx = editIdx.value;
    const dt = new Date(), day = getDay(dt), month = getMonthStr(dt);
    if (edIdx !== '') {
      edIdx = +edIdx;
    ...
    lastUsedCategory = category; localStorage.setItem('lastUsedCategory', category);
    closeMainModal(); renderAll();
    return false;
  };
  closeModal.onclick = closeMainModal;
  modal.onclick = e => { if (e.target === modal) closeMainModal(); };
  ...
  function deleteExpense(idx) {
    lastDeleted = expenses[idx]; lastDeletedIndex = idx;
  ...
  undoBtn.onclick = () => {
    if (lastDeleted) {
      ...
      localStorage.setItem('expenses', JSON.stringify(expenses));
      renderAll();
      lastDeleted = null; lastDeletedIndex = null;
  ...
  openSummaryBtn.addEventListener('click', openSummaryPanel);
  closeSummaryBtn.addEventListener('click', closeSummaryPanel);
  ...
  function renderSummaryPanel() {
    ...
    const filtered = expenses.filter(e =>
      ...
      (
        ...
        (e.amount && e.amount.toString().includes(search))
      )
    );
    const total = filtered.reduce((acc, e) => acc + Number(e.amount), 0);
    summaryAmount.innerHTML = `
      <div class="summary-inline-amount">₹${total}</div>
    `;
    summaryPanelBody.innerHTML = filtered.length === 0
  ...
  function renderAll() {
    ...
    let monthsObj = {};
    list.forEach(e => { if (!monthsObj[e.month]) monthsObj[e.month] = []; monthsObj[e.month].push(e); });
  ...
  function renderChart() {
    ...
    const months = Object.keys(byMonth).sort((a, b) => {
      return (new Date('1 ' + b.split(' ') + ' ' + b.split(' '))) - (new Date('1 ' + a.split(' ') + ' ' + a.split(' ')));
    });
    ...
    chart = new Chart(chartCanvas, {
      ...
      options: {
        ...
        scales: { x: { stacked: true }, y: { beginAtZero: true, grace: '14%' } }
      }
    });
  }
